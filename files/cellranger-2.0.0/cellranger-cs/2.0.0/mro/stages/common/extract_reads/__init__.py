...
def main(args, outs):
    ...
    for extractions in itertools.islice(all_read_iter, args.initial_reads):
        ...
        # Verify the non-emptiness of the RNA read(s)
        if len(rna_read[1]) == 0 and rna_read_def.read_type is not None:
            martian.exit('A non-empty sequence was expected on the %s read, but an empty sequence was encountered. Please verify that the sequencing configuration was correct and that the correct library type was provided as input. This error can occur when Single Cell 3\' data are supplied to the Single Cell V(D)J pipeline.' % rna_read_def.read_type)
        if len(rna_read2[1]) == 0 and rna_read2_def.read_type is not None:
            martian.exit('A non-empty sequence was expected on the %s read, but an empty sequence was encountered. Please verify that the sequencing configuration was correct and that the correct library type was provided as input. This error can occur when Single Cell 3\' data are supplied to the Single Cell V(D)J pipeline.' % rna_read2_def.read_type)
