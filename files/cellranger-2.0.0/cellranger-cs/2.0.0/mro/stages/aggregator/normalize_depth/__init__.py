...
stage NORMALIZE_DEPTH(
    in  h5     molecules,
    in  int[]  cells_per_gem_group,
    in  int[]  cmb_reads_per_gem_group,
...
def split(args):
    ...
    with cr_mol_counter.MoleculeCounter.open(args.molecules, 'r') as mol_counter:
        for (gg, submetrics) in mol_counter.get_metric(cr_mol_counter.GEM_GROUPS_METRIC).iteritems():
            ...
            total_reads_per_gem_group.append(submetrics[cr_mol_counter.GG_TOTAL_READS_METRIC])

    effective_reads_per_gem_group = total_reads_per_gem_group if use_raw_reads else args.cmb_reads_per_gem_group
    assert len(effective_reads_per_gem_group) == len(args.cells_per_gem_group)
    rpcs = [tk_stats.robust_divide(reads, cells) if cells > 0 else 0.0 for (reads, cells) in zip(effective_reads_per_gem_group, args.cells_per_gem_group)]

    lowest_rpc = min(rpcs)
    downsample_map = {}
    for gg, rpc, cmb_reads, total_reads, cells in zip(gem_groups, rpcs, args.cmb_reads_per_gem_group, total_reads_per_gem_group, args.cells_per_gem_group):
        downsample_map[gg] = {'total_reads': total_reads, 'cmb_reads': cmb_reads, 'cells': cells, 'frac_reads_kept': 1.0}
        if downsample and len(gem_groups) > 1:
            if lowest_rpc == 0:
                # one or more samples are empty. just do the naive thing for now.
                frac_reads_kept = 0.0
            else:
                frac_reads_kept = tk_stats.robust_divide(lowest_rpc, rpc)
            downsample_map[gg]['frac_reads_kept'] = frac_reads_kept
