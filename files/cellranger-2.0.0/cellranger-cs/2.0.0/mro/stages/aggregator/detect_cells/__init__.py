...
stage DETECT_CELLS(
    ...
    out int[] cells_per_gem_group,
    out int[] cmb_reads_per_gem_group,
...
def main(args, outs):
    ...
    with cr_mol_counter.MoleculeCounter.open(args.molecule_h5, 'r', start=int(args.chunk_start), length=int(args.chunk_len)) as ctr_in:
        ...
        outs.cells_per_gem_group = [int(total_cells)]
        outs.cmb_reads_per_gem_group = [int(total_conf_mapped_cell_reads)]
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    gg_stats = [(cd.gem_group, co.cells_per_gem_group[0], co.cmb_reads_per_gem_group[0]) for (cd, co) in zip(chunk_defs, chunk_outs)]
    outs.cells_per_gem_group = [int(cells) for (gem, cells, reads) in sorted(gg_stats)]
    outs.cmb_reads_per_gem_group = [int(reads) for (gem, cells, reads) in sorted(gg_stats)]
