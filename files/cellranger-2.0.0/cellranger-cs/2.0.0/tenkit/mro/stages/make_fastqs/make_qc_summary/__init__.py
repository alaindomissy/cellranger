...
class ReadQCStats(object):
    ...
    def total_gem_estimate(self, threshold=GEM_COUNT_THRESHOLD_ESTIMATE):
        return len([val for val in self.unique_barcode_counts.values() if val > threshold])
...
def join(args, outs, chunk_args, chunk_outs):
    ...
    for chunk_arg, chunk_out in zip(chunk_args, chunk_outs):
        lane_sample_stats = lane_sample_info[(chunk_arg.lane, chunk_arg.sample)]
        sample_stats = sample_info[chunk_arg.sample]
